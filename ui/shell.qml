...
FloatingWindow {
    ...
    readonly property var keymaps: ({
        "channel": {
            ...
            "a":        { act: () => { if (Backend.messageActionEnabled && focusedPanel === "messages") Backend.runMessageAction(msgs.currentMessage()) }, help: () => Backend.messageActionEnabled ? "Custom message action" : "", cat: "msg" },
